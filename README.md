...
- **Code-level clearance gate.** The prompt asked the LLM to honor "Secret clearance required"; flash dropped that instruction unreliably. A deterministic substring scan caps the score at 1.5 regardless of stack overlap. The federal-sector FDE record (requiring Secret clearance) dropped from +1.75 over-score to label-aligned.
